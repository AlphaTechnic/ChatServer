...
#### 서버 (ChatServer)
  ...
  * [x] **(선택) 랜덤 방 입장:** 클라이언트의 랜덤 방 입장 요청 시, 입장 가능한 방에 랜덤으로 입장합니다.
  * [x] **(선택) 클라이언트 종료 관리:** 일정 시간(60초) 무응답 클라이언트는 세션을 정리(Timeout)합니다.
  * [x] **(선택) DB 채팅 기록:** 모든 채팅을 인메모리 DB(`std::deque`)에 기록하며, 7일이 지난 로그는 자동 삭제합니다.
